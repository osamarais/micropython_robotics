	...
	for intr in range(lr):
		for intr2 in range(lr):
			if intr!=intr2:
				...
				angmat[intr][intr2]=-90+(-90-angmat[intr][intr2])
...
def inourout(boundlist,lr,numrobot):
	...
	print(mindis[countmin],state,countmin,escangle)
...
for k in range(l):
	bound_angle[k]=180-math.degrees(math.atan2(inedgepos[k][1]-inrobpos[first][1],inedgepos[k][0]-inrobpos[first][0]))
	if bound_angle[k]<0:
		bound_angle[k]+=360
	if bound_angle[k]<0:
		bound_angle[k]+=360
...
countmax=0
for maxx in range(l):
	if bound_angle[maxx]==max(bound_angle):
		break
	countmax+=1
...
angaim=math.pi-math.atan2(inedgepos[passage+1][1]-inedgepos[passage][1],inedgepos[passage+1][0]-inedgepos[passage][0])#-math.pi/2
...
while True:
	...
	if preycount==0:
		preyangle=180-math.degrees(math.atan2(aim[1]-preypos[1],aim[0]-preypos[0]))
