...
while True:
	...
	while True:
		...
		for lii in range(lr-1):
			c1.append([0,0,0])


		#c1=len(roblist)*[[0]*3]
		#c2=len(boundlist)*[[0]*3]


		c2=[[0,0,0]]

		for lia in range(l-1):
			c2.append([0,0,0])
		...
		for check2 in range(len(boundlist)):
			c2[check2][0],c2[check2][1],c2[check2][2]=dedector.Locate(boundlist[check2])


		if not None in [y for x in c1 for y in x] and not None in [y for x in c2 for y in x]:
		#if not None in (X15, X11, X5, X10, Y16, X6):
			break
...
######################################
	...
	aa,bb,cc,dd=inourout(boundlist,lr,1)
	print(bb)
...
######################################
	...
	#SEND DATA
	data = [preyangle,ll,bb]
	senddata(data,IP,port,sock)
